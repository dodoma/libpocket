...
char* mfile_test()
{
    ...
    FILE *fp = fopen("/data/user/0/com.example.libmoc_example/app_flutter/avmISAVM.txt", "w");
    if (fp) {
        fprintf(fp, "xxxyyyzzz %d\n", count++);
        fclose(fp);
    } else TINY_LOG("open file for write failure %s", strerror(errno));

    fp = fopen("/data/user/0/com.example.libmoc_example/app_flutter/avmISAVM.txt", "r");
    if (fp) {
